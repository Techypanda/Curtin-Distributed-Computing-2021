...
namespace SimpleLibrary {
    public class SimpleP2PChain {
        ...
        static SimpleP2PChain() {
            GenerateGenesisBlock();
        }

        public void SetChain(List<Block> inChain) {
            _blockchain = inChain;
        }
            ...
            if (CheckHashValid(block)) {
                _blockchain.ForEach((Block blockInDB) => {
                    if (blockInDB.BlockID >= block.BlockID) {
                        throw new P2PChainException("The Block's ID is not valid");
                    }
                });
                if (CalculateWalletCoins(block.WalletIDFrom) >= block.Amount) {
                    if (block.Amount <= 0) {
                        throw new P2PChainException("The Block's Amount Cannot Be Less Than Or Equal To 0");
                    }
                    if (block.BlockOffset % 5 != 0) {
                        throw new P2PChainException("The Block's Offset Is Incorrect");
                    }
                    if (!block.PreviousHash.Equals(_blockchain.Last().Hash)) {
                        throw new P2PChainException("The Previous Hash Is Invalid");
                    }
                    _blockchain.Add(block);
                } else {
                    throw new P2PChainException("The Block's From Wallet Does Not Have Sufficient Funds.");
                }
            } else {
                throw new P2PChainException("The Block's Hash Is Not Valid");
            }
        }
